...
def response(A0, Omega, t, Phi0, psi, iota, l1, l2, h_ab):
    ...
    h_i = 0.5*h_ab*(np.einsum('ki,kj->kij', l1, l1)- np.einsum('ki,kj->kij', l2, l2)) #response at LISA for WD q at time j
    return h_i
...
for q in np.arange(N_WD):
    h_i=response(A0[q,0], Omega[q,0], time, Phi0[q,0], psi[q,0], iota[q,0], l_i[:,0], l_i[:,1], h_ab2)
...
freq=np.arange(0, df*len(h_I), df)
fig = plt.figure()
plt.plot(time, h_mod)
plt.xlabel('t (s)')
plt.ylabel('|h|')
